...
def integra_file_maker(plate_dimensions_df, output_dir):
    ...
    for index, row in plate_dimensions_df.iterrows():
        ...
        column_gap = round_mm((((row['plate_length_mm'] - row['offset_left_to_a1_mm'] - row['offset_right_to_final_well_mm']) - (row['num_columns'] * row['well_length_mm']) )/ (row['num_columns'] - 1)) + row['well_length_mm'])
        row_gap = round_mm((((row['plate_width_mm'] - row['offset_top_to_a1_mm'] - row['offset_bottom_to_final_well_mm']) - (row['num_rows'] * row['well_width_mm'])) / (row['num_rows'] - 1)) + row['well_width_mm'])
        ...
        for tag, text in [
            ...
            ('Depth', depth),
            ('NominalWellVolume', round_mm(row['max_volume_ul'])),
